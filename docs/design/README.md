...
The design system's component **source** (`components/**/*.jsx`). The manifest
names 18 components and their paths, but only the compiled bundle shipped. How
this project consumes the design system is a decision recorded in `docs/adr/`.
...
Treat every backend claim in it as a guess and check `D:\Projects\aod-backend`.
